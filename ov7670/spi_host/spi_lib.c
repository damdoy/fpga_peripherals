...
int spi_send(uint8_t cmd, uint8_t val[32], uint8_t *status)
{
   // uint8_t to_send[33] = {cmd, val[0], val[1], val[2]};
   uint8_t to_send[33];
   to_send[0] = cmd;
...
int spi_send24b(uint8_t cmd, uint32_t val24b, uint8_t *status){
   uint8_t param[32];
   param[0] = val24b&0xff;
   param[1] = (val24b>>8)&0xff;
   param[2] = (val24b>>16)&0xff;
   return spi_send(cmd, param, status);
}

int spi_read(uint8_t val[32], uint8_t *status)
{
   uint8_t nop_command[33]; //nop
   uint8_t status_recv = 0;
   uint32_t retries = 0;
